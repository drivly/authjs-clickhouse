...
async function hashToken(token: string): Promise<string> {
  ...
  const data = encoder.encode(token + 'anything')
...
export async function runBasicTests(options: TestOptions) {
  ...
  const maybeTest = (method: keyof Adapter, ...args: Parameters<typeof test> extends [any, ...infer U] ? U : never) =>
  ...
  const customModel = typeof window === 'undefined' && globalThis.process?.env?.CUSTOM_MODEL === '1'
  if (customModel) {
    user.role = 'admin'
  ...
  maybeTest('listAuthenticatorsByUserId', async () => {
    ...
    expect([authenticator2, authenticator1]).toEqual(expect.arrayContaining(authenticators1 || []))
  ...
  maybeTest('updateAuthenticatorCounter', async () => {
    ...
    // Test
    await expect(() => adapter.updateAuthenticatorCounter('invalid-credential-id', randomInt(100))).rejects.toThrow()
